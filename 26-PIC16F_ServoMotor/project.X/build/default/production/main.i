...
void __attribute__((picinterrupt(("")))) ISR(void) {
    if (TMR1IF) {
        ...
        PORTE |= (1 << servo.pin);
        for (uint16_t i = 0; i < servo.pulseWidth; i++) __asm("nop");
        PORTE &= ~(1 << servo.pin);
...
void ServoTimer2(void) {
    initISR();
    servo.pulseWidth = 165;
    servo.pin = 0;
    TRISE &= ~(1 << servo.pin);
    PORTE &= ~(1 << servo.pin);
}
...
void write(int pulseWidth) {
    if (pulseWidth < 109) pulseWidth = 109;
    if (pulseWidth > 221) pulseWidth = 221;
...
void main(void) {
    ...
    while (1) {
        ...
        voltage = (adcResult * 5.0) / 1023.0;
        pulseWidth = (uint16_t)((voltage * (221 - 109)) / 5.0) + 109;
        angle = (pulseWidth - 109) * 180.0 / (221 - 109) - 90.0;
