...
int32_t _last_temp = 55555;
int32_t _last_rh = 1111;
uint8_t last_conversion_type = 0;

uint8_t TH02_WriteRegister(uint8_t reg, uint8_t value) {
    I2C_Master_Start();
    if (I2C_Master_Write(0x40 << 1) != 0) return 1;
...
uint8_t TH02_ReadRegister(uint8_t reg, uint8_t *value) {
    ...
    if (I2C_Master_Write((0x40 << 1) | 0x01) != 0) return 1;
...
uint8_t TH02_StartTempConversion(uint8_t fastmode, uint8_t heater) {
    uint8_t config = 0x01 | 0x10;
    if (fastmode) config |= 0x20;
    if (heater) config |= 0x02;
    return TH02_WriteRegister(3, config);
    }
...
uint8_t TH02_WaitForReady(void) {
    ...
    while (time_out <= 100) {
        if (TH02_ReadRegister(0, &status) == 0) {
...
int16_t TH02_GetCompensatedRH(uint8_t round_flag) {
    ...
    if (_last_rh != 1111) {
        ...
        rhlinear = rhvalue - ((rhvalue * rhvalue) * -0.00393f +
                rhvalue * 0.4008f +
                -4.7844f);

        if (_last_temp != 55555) {
            rhlinear += ((_last_temp / 100.0f) - 30.0f) *
                    (rhlinear * 0.00237f + 0.1973f);
            }
...
int16_t TH02_GetConversionValue(void) {
    ...
    I2C_Master_Start();
    if (I2C_Master_Write((0x40 << 1)) != 0) {
        I2C_Master_Stop();
        return ret;
        }


    if (I2C_Master_Write(1) != 0) {
        I2C_Master_Stop();
        return ret;
        }
    ...
    I2C_Master_RepeatedStart();
    if (I2C_Master_Write((0x40 << 1) | 0x01) != 0) {
        I2C_Master_Stop();
        return ret;
        }
    ...
    if (TH02_ReadRegister(3, &config) == 0) {
        if (config & 0x10) {
