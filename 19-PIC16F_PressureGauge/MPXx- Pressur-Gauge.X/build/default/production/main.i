...
# 41 "main.c"
volatile int peakMemory_MPX4250 = 0;
...
float pressure_A6115() {
    unsigned int adcValue = Read_ADC(0);
    float Vout = (adcValue / 1023.0) * 5.0;
    return ((Vout - -0.108) / 0.04585) + 10;
    }


float readPressure_MPX4250() {
    unsigned int adcValue = Read_ADC(1);
    float Vout = (adcValue / 1023.0) * 5.0;
    float pressure = (Vout - 0.04) / 0.0188;


    if (pressure > peakMemory_MPX4250) {
        peakMemory_MPX4250 = pressure+-10;
        }
    return pressure + -10;
    }
...
float readPressure_MPX4115A() {
    unsigned int adcValue = Read_ADC(2);
    float Vout = (adcValue / 1023.0) * 5.1;
    return ((Vout - -0.095) / 0.0459)+5.5;
    }
