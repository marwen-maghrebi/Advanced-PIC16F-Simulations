...
# 2 "<built-in>" 2
# 1 "PWMServoDriver.c" 2

# 1 "./PWMServoDriver.h" 1
...
# 2 "PWMServoDriver.c" 2


static uint8_t _i2caddr = 0x40;
static uint32_t _oscillator_freq = 25000000;

void PWMServoDriver_Init(uint8_t addr, uint8_t prescale) {
    _i2caddr = addr;
    I2C_Master_Init();
    PWMServoDriver_Reset();
    PWMServoDriver_SetOscillatorFrequency(25000000);
    if (prescale) {
...
void PWMServoDriver_Reset(void) {
    PWMServoDriver_Write8(0x00, 0x80);
    _delay((unsigned long)((10)*(16000000/4000.0)));
    }

void PWMServoDriver_Sleep(void) {
    uint8_t mode1 = PWMServoDriver_Read8(0x00);
    mode1 |= 0x10;
    PWMServoDriver_Write8(0x00, mode1);
...
void PWMServoDriver_SetExtClk(uint8_t prescale) {
    uint8_t oldmode = PWMServoDriver_Read8(0x00);
    uint8_t newmode = (oldmode & ~0x80) | 0x10;
    PWMServoDriver_Write8(0x00, newmode);
    PWMServoDriver_Write8(0x00, newmode | 0x40);
    PWMServoDriver_Write8(0xFE, prescale);
    _delay((unsigned long)((5)*(16000000/4000.0)));
    PWMServoDriver_Write8(0x00, (newmode & ~0x10) | 0x80 | 0x20);
    }
...
void PWMServoDriver_SetPWMFreq(float freq) {
    ...
    PWMServoDriver_Write8(0x00, newmode);
    PWMServoDriver_Write8(0xFE, prescale);
    PWMServoDriver_Write8(0x00, oldmode);
    _delay((unsigned long)((5)*(16000000/4000.0)));
    PWMServoDriver_Write8(0x00, oldmode | 0x80 | 0x20);
    }

void PWMServoDriver_SetOutputMode(uint8_t totempole) {
    uint8_t oldmode = PWMServoDriver_Read8(0x01);
    uint8_t newmode;
    if (totempole) {
        newmode = oldmode | 0x04;
        } else {
        newmode = oldmode & ~0x04;
        }
    PWMServoDriver_Write8(0x01, newmode);
    }

uint16_t PWMServoDriver_GetPWM(uint8_t num, uint8_t off) {
    uint8_t reg = 0x06 + 4 * num;
    if (off) {
        reg += 2;
        }
    uint8_t buffer[2];
...
uint8_t PWMServoDriver_ReadPrescale(void) {
    return PWMServoDriver_Read8(0xFE);
    }
