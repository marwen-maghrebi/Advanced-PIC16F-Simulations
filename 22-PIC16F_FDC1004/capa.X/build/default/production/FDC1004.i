...
void FDC1004_Write16(FDC1004 *fdc, uint8_t reg, uint16_t data) {
    I2C_Master_Start();
    I2C_Master_Write((fdc->address << 1));
...
uint16_t FDC1004_Read16(FDC1004 *fdc, uint8_t reg) {
    ...
    I2C_Master_Start();
    I2C_Master_Write((fdc->address << 1) | 1);
    value = I2C_Read_Byte();
    I2C_ACK();
...
uint8_t FDC1004_ConfigureMeasurement(FDC1004 *fdc, uint8_t measurement, uint8_t channel, uint8_t capdac) {
    if (!(measurement >= 0 && measurement <= (0x03)) || !(channel >= 0 && channel <= (0x03)) || capdac > (0x1F)) {
        return 1;
        }
...
uint8_t FDC1004_TriggerMeasurement(FDC1004 *fdc, uint8_t measurement, uint8_t rate) {
    if (!(measurement >= 0 && measurement <= (0x03)) || !(rate == (0x01) || rate == (0x02) || rate == (0x03))) {
        return 1;
        }
    uint16_t trigger = (rate << 10) | (1 << (7 - measurement));
    FDC1004_Write16(fdc, (0x0C), trigger);
    return 0;
    }

uint8_t FDC1004_ReadMeasurement(FDC1004 *fdc, uint8_t measurement, uint16_t *value) {
    if (!(measurement >= 0 && measurement <= (0x03))) {
        return 1;
        }
    uint16_t fdc_register = FDC1004_Read16(fdc, (0x0C));
...
int32_t FDC1004_GetCapacitance(FDC1004 *fdc, uint8_t channel) {
    ...
    if (FDC1004_GetRawCapacitance(fdc, channel, &result)) return (-1-0x7fffffff);
    int32_t capacitance = (int32_t)result.value * (488) / 1000;
    capacitance += (int32_t)result.capdac * (3125);
    return capacitance;
    }

uint8_t FDC1004_GetRawCapacitance(FDC1004 *fdc, uint8_t channel, fdc1004_measurement_t *result) {
    if (!(channel >= 0 && channel <= (0x03))) return 1;
    ...
    while (result->value > 0x7E00 || result->value < 0x8100) {
        ...
        if (result->value > ((int16_t) 0x4000) && result->capdac < (0x1F)) {
            result->capdac++;
            } else if (result->value < (-1 * ((int16_t) 0x4000)) && result->capdac > 0) {
                result->capdac--;
                } else {
